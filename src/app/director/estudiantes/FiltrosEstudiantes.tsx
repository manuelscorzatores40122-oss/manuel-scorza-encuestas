...
export function FiltrosEstudiantes({
  ...
  // Opciones restringidas en cascada (100 % cliente, sin roundtrip)
  const gradosFiltrados = nivel
  ...
  function push(overrides: Partial<{ q: string; nivel: string; gradoId: string; seccionId: string }>) {
    ...
    startTransition(() => router.push(`/director/estudiantes?${params}`));
  }

  function onNivel(val: string) {
    setNivel(val); setGradoId(''); setSeccionId('');
    push({ nivel: val, gradoId: '', seccionId: '' });
  }
  function onGrado(val: string) {
    setGradoId(val); setSeccionId('');
    push({ gradoId: val, seccionId: '' });
  }
  function onSeccion(val: string) {
    setSeccionId(val);
    push({ seccionId: val });
  }
  function onSearch() {
  ...
  function onClear() {
    setQ(''); setNivel(''); setGradoId(''); setSeccionId('');
    router.push('/director/estudiantes');
  }
  ...
  return (
    <div className={`${styles.filterForm} ${isPending ? styles.filterPending : ''}`}>
      ...
      {/* ── Fila inferior: selects en cascada ── */}
      <div className={styles.filterRow}>
        ...
        <div className={styles.filterGroup}>
          ...
          <select
            value={gradoId}
            onChange={e => onGrado(e.target.value)}
        ...
        <div className={styles.filterGroup}>
          ...
          <select
            ...
            className={styles.filterSelect}
            disabled={seccionesFiltradas.length === 0}
          >
          ...
          <div className={styles.filterChips}>
            {nivel && (
              <span className={styles.chip}>
                ...
                <button onClick={() => onNivel('')}><X size={11} /></button>
              </span>
            )}
            {gradoId && (
              <span className={styles.chip}>
                {gradosFiltrados.find(g => g.id === gradoId)?.name ?? 'Grado'}
                <button onClick={() => onGrado('')}><X size={11} /></button>
              </span>
            )}
            {seccionId && (
              <span className={styles.chip}>
                Secc. {seccionesFiltradas.find(s => s.id === seccionId)?.name ?? '?'}
                <button onClick={() => onSeccion('')}><X size={11} /></button>
              </span>
            )}
          </div>
        )}
